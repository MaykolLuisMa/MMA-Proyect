...
def objetive_function(variables, tasks : List[Task], hours : List):
    return lpSum(variables[(t,h)]*lpSum(p.hours_preference[h] for p in t.professors)/len(t.professors) for t in tasks for h in hours)

def reestriction1(problem : LpProblem,variables, tasks : List[Task], hours : List, professors):
    for p in professors:
        for h in hours:
            problem += lpSum(variables[(t,h)] for t in tasks if p in t.professors) <= 1,f"restriction1{p},{h}"        
    ...
    for h in hours:
        for t in tasks:
            for p in t.professors:
                problem += p.hours_preference[h] >= variable[(t,h)] ,f"restriction2{t},{h},{p}"                
...
def reestriction4(problem : LpProblem, variable, tasks : List[Task], hours : List, all_thesis : List[Thesis]):
    for w in all_thesis:
        ...
        print(f"W tasks: {len(w_tasks)}")
        if len(w_tasks) == 10:
            print(w.title)
        for h in hours:
            problem += lpSum(variable[(t,h)] for t in w_tasks) == 5*variable[(w_tasks[0],h)],f"restriction4,{w},{h}"

def reestriction5(problem : LpProblem,variable, tasks : List[Task], hours : List):
    for t in tasks:
        problem += lpSum(lpSum(variable[(t,h)] for h in hours))==1 ,f"restriction5{t}"                
